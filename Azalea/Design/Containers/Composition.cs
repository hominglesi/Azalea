...
public partial class Composition : GameObject
{
	public Composition()
	{
	...
	private List<GameObject> _publicChildren = [];

	public virtual IReadOnlyList<GameObject> Children
	{
		get => _publicChildren;
		set
		{
	...
	{
		var children = _publicChildren.ToArray();
	...
	{
		if (gameObject == null)
			throw new ArgumentNullException(nameof(gameObject), $"Cannot add null {nameof(gameObject)} to {nameof(Composition)}.");

		if (gameObject == this)
			throw new InvalidOperationException($"{nameof(Composition)} cannot be addet to itself.");
	...
	protected internal int IndexOfInternal(GameObject gameObject)
	{
		if (gameObject.Parent != null && gameObject.Parent != this)
			throw new InvalidOperationException($@"Cannot call {nameof(IndexOfInternal)} for a {nameof(GameObject)} that already is a child of a different parent");
	...
	protected internal void ChangeInternalChildDepth(GameObject child, float newDepth)
	{
		...
		if (index < 0)
			throw new InvalidOperationException($"Can not change depth of an object which is not contained within this {nameof(Composition)}.");
	...
	protected class ChildComparer(Composition owner) : IComparer<GameObject>
	{
		private readonly Composition _owner = owner;
